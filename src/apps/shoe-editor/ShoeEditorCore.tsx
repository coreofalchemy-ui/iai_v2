...
import { CloseIcon, BeautifyIcon, StudioIcon, NaturalLightIcon, SideLightingIcon, CinematicIcon, CustomIcon } from './components/Icons'; // Assuming Icons are exported or I need to define them here.

// Re-defining icons here to avoid import issues if they are not in a separate file
const Icons = {
    NaturalLight: () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" /></svg>,
    Cinematic: () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15 5v14M9 5v14M5 9h4m0 0v6m0-6h6m-6 0v6m6-6h4m0 0v6m-4-6v-2a2 2 0 012-2h0a2 2 0 012 2v2m-4 6v2a2 2 0 002 2h0a2 2 0 002-2v-2" /></svg>,
    SideLighting: () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M12 12m-4 0a4 4 0 1 0 8 0a4 4 0 1 0 -8 0" /><path d="M3 12h6" /><path d="M15 12h6" /></svg>,
    Studio: () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg>,
    Beautify: () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>,
    Custom: () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>,
    Close: () => <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg>
};
...
const effects: { id: Effect; name: string; icon: React.ReactElement }[] = [
    { id: 'beautify', name: '미화 (제품 누끼)', icon: <Icons.Beautify /> },
    { id: 'studio_minimal_prop', name: '스튜디오 - 미니멀 소품', icon: <Icons.Studio /> },
    { id: 'studio_natural_floor', name: '스튜디오 - 자연광 바닥', icon: <Icons.NaturalLight /> },
    { id: 'studio_texture_emphasis', name: '스튜디오 - 텍스처 부각', icon: <Icons.SideLighting /> },
    { id: 'studio_cinematic', name: '스튜디오 - 시네마틱', icon: <Icons.Cinematic /> },
    { id: 'custom', name: '커스텀 배경', icon: <Icons.Custom /> },
];
...
interface ShoeEditorCoreProps {
    initialFiles?: File[];
    onApply?: (results: Result[]) => void;
}

export const ShoeEditorCore: React.FC<ShoeEditorCoreProps> = ({ initialFiles = [], onApply }) => {
    ...
    useEffect(() => {
        if (initialFiles.length > 0) {
            ...
            const newFiles = initialFiles.filter(f => !uploadedFiles.some(uf => uf.name === f.name));
    ...
    return (
        <div className="flex flex-col lg:flex-row h-full">
                ...
                <div className="p-6 space-y-6 flex-1">
                    ...
                    <div className="mt-auto pt-8">
                        <button
                            ...
                            className="main-button bg-blue-600 text-white transition-colors hover:bg-blue-700 disabled:bg-zinc-400 disabled:cursor-not-allowed w-full py-4 rounded-lg font-bold text-lg"
                        >
            ...
            {/* Right Content Area */}
            <main className="flex-1 bg-[#FAFAFA] p-8 overflow-y-auto">
                {results.length === 0 ? (
                    ...
                    <div>
                            ...
                            <div className="flex gap-2 items-center flex-wrap">
                                ...
                                {onApply && (
                                    <button
                                        ...
                                        className="bg-green-600 text-white font-bold py-3 px-6 rounded-lg transition-colors hover:bg-green-700 disabled:bg-zinc-400"
                                    >
                                        상세페이지에 적용 ({selectedResultsCount})
                                    </button>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                            {results.map(result => (
                                <div
                                    key={result.id}
                                    className={`relative bg-white rounded-xl overflow-hidden shadow-sm border-2 transition-all cursor-pointer group ${result.isSelected ? 'border-blue-500 ring-4 ring-blue-100' : 'border-transparent hover:border-zinc-300'}`}
                                ...
                                >
                                    <div className="aspect-[3/4] relative bg-zinc-100">
                                        {result.status === 'done' && result.url ? (
                                            <>
                                                <img
                                                    ref={el => imageRefs.current[result.id] = el}
                                                    src={result.url}
                                                    alt="Generated"
                                            ...
                                            <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
                                                <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-red-400 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                                <p className="text-red-500 font-medium text-sm">{result.error}</p>
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); handleRegenerate(result.id); }}
                                                    className="mt-3 text-xs bg-white border border-red-200 text-red-600 px-3 py-1.5 rounded-full hover:bg-red-50"
                                                >
                                                    다시 시도
                                                </button>
                                            </div>
                                            ...
                                            <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/80 backdrop-blur-sm z-10">
                                                ...
                                                <p className="text-blue-600 font-medium animate-pulse">{result.processingStep || '처리 중...'}</p>
                                            </div>
                                        )}

                                        {/* Selection Checkbox */}
                                        <div className={`absolute top-3 left-3 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${result.isSelected ? 'bg-blue-500 border-blue-500' : 'bg-white/50 border-white'}`}>
                                            {result.isSelected && <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-white" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>}
                                        </div>
                                    </div>

                                    <div className="p-4 border-t border-zinc-100">
                                        <div className="flex justify-between items-start mb-1">
                                            <span className="inline-block px-2 py-1 bg-zinc-100 text-zinc-600 text-xs font-medium rounded-md">
                                                {effects.find(e => e.id === result.effect)?.name}
                                            </span>
                                            {result.poseInfo && (
                                                <span className="inline-block px-2 py-1 bg-blue-50 text-blue-600 text-xs font-medium rounded-md ml-2">
            ...
            {modalData && (
                <ComparisonModal
                    ...
                    generatedUrl={modalData.generatedUrl}
